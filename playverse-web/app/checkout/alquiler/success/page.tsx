...
export default function RentalSuccessPage() {
  const [payload, setPayload] = useState<RentalPayload | null>(null);

  useEffect(() => {
    try {
      const raw = typeof window !== "undefined" ? sessionStorage.getItem("pv_rental_success") : null;
      if (raw) {
        const p = JSON.parse(raw) as RentalPayload;
        setPayload(p);
        sessionStorage.removeItem("pv_rental_success");
      }
    } catch {
      setPayload(null);
    }
    ...
    <div className="min-h-screen bg-slate-900 text-white flex items-center justify-center p-4">
      <div className="max-w-3xl w-full text-center">
        <div className="bg-slate-800/50 border border-orange-400/30 rounded-lg p-8">
          ...
          <h1 className="text-2xl font-bold text-white mb-4">¡Alquiler confirmado!</h1>
          ...
          {payload ? (
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-center mb-6">
              ...
              <div className="md:col-span-2 text-left">
                ...
                <p className="text-slate-300 mb-3">Gracias, tu alquiler fue activado correctamente.</p>
                ...
                <p className="text-slate-400 mt-4 text-sm">Te enviaremos un correo con los detalles y el comprobante a la cuenta asociada.</p>
              </div>
            </div>
          ) : (
            <div className="text-slate-300 mb-6">No se encontraron los detalles del alquiler. Si el pago fue exitoso, tu alquiler debería aparecer en "Mis juegos" o en la sección de alquileres.</div>
          )}
