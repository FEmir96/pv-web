...
export default function PurchaseSuccessPage() {
  ...
  useEffect(() => {
    try {
      const raw = typeof window !== "undefined" ? sessionStorage.getItem("pv_purchase_success") : null;
      if (raw) {
        const p = JSON.parse(raw) as PurchasePayload;
        setPayload(p);
        // opcional: limpiar el storage para no reutilizarlo
        sessionStorage.removeItem("pv_purchase_success");
      }
    } catch {
      setPayload(null);
    }
          ...
          {payload ? (
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-center mb-6">
              ...
              <div className="md:col-span-2 text-left">
                ...
                <p className="text-slate-300 mb-3">Gracias por tu compra. Tu juego ya fue agregado a tu biblioteca.</p>
                ...
                <p className="text-slate-400 mt-4 text-sm">Te enviaremos un correo con los detalles de la compra y el recibo a la cuenta asociada.</p>
              </div>
            </div>
          ) : (
            <div className="text-slate-300 mb-6">No se encontraron los detalles de la compra. Si el pago fue exitoso, tu juego debería aparecer en "Mis juegos".</div>
          )}
